...
namespace _02.Scripts.UI.Title
{
    ...
    /// UX 스타일 열거형
    /// </summary>
    public enum ChoiceUXType { FocusOnly, Continuous }

    /// <summary>
    /// [V] 개별 타이틀 버튼의 Shapes 외형을 담당하는 패널.
    /// 설정에 따라 두 가지 UX 스타일(단순 포커스 확대 / 거리 기반 부드러운 확대)을 지원합니다.
    /// </summary>
    [ExecuteAlways]
    public class TitleButtonPanel : ImmediateModePanel, IPointerEnterHandler, IPointerExitHandler, IPointerDownHandler, IPointerUpHandler
    {
        ...
        private ChoiceUXType m_CurrentUXType = ChoiceUXType.FocusOnly;
        ...
        public void SetFocus(bool focused)
        {
            ...
            m_IsFocused = focused;
            if (m_CurrentUXType == ChoiceUXType.FocusOnly)
            {
        ...
        public void SetUXState(bool focused, float distance, ChoiceUXType uxType)
        {
            ...
            m_CurrentUXType = uxType;

            if (m_CurrentUXType == ChoiceUXType.FocusOnly)
            {
                m_TargetScale = focused ? Vector3.one * focusScale : Vector3.one;
            }
            else // Continuous 모드: 거리에 따라 가변적으로 확대
            {
                // 거리가 0에 가까울수록(중앙일수록) focusScale에 가까워짐
        ...
        public override void DrawPanelShapes(Rect rect, ImCanvasContext ctx)
        {
            ...
            else
            {
                ...
                else if (m_IsFocused || (m_CurrentUXType == ChoiceUXType.Continuous && m_NormalizedDistance < 0.3f)) 
